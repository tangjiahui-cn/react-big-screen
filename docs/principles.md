...
同一规则、流程、约定或业务知识只保留一个权威载体，其他位置通过引用或调用复用，确保变化只需修改一处。载体既可以是代码，也可以是人写的文字——注释、文档、提交信息。代码中，仅对语义一致、变化点相同且重复出现的稳定逻辑进行抽象，并遵循 Rule of Three，避免因代码相似而过早提取。

### 不复述别处已有的事实

注释、文档等文字载体只承载没有其他载体的信息。代码行为由代码本身自证，能通过命名、类型和代码结构清晰表达的内容不需要再写一遍；设计意图、业务约束和特殊原因没有别的载体，必须写下来。仅在缺少文字会导致后续维护者误解意图、错误修改或无法理解特殊约束时才添加，避免无意义、重复或过度的文字。

### 不残留已经失效的事实

文字必须与它描述的对象保持一致；当对象发生变化时，应同步更新或删除失效文字，避免文字成为第二套过期事实来源。

...
react-big-screen 是一个基于 React 从 0 到 1 开发的 `可视化编辑器` 项目，通过简单的拖拽和配置即可搭建页面。本项目已实现多数核心功能，包括：拖拽系统、自定义组件、多页面管理、组件联动、远程组件、自适应预览等（详见下文）。

> Tips: AI 搭建能力开发中 ...（2026.05.22）
...
本项目集成了完整的 AI 辅助开发流水线。通过 [Claude Code](https://claude.ai/code) CLI 配合 `pipeline` 技能，你只需描述需求，即可自动完成规格设计、实施计划、代码编写和测试验证。

### 工作流程

| 步骤 | 操作 | 说明 |
|------|------|------|
| **1. 打开终端** | 在项目根目录下打开终端 | 确保已在项目目录中 |
| **2. 启动 Claude CLI** | 运行 `claude` 命令 | 进入 Claude Code 交互环境 |
| **3. 提交需求** | 使用 `/pipeline <你的需求描述>` | 描述你想要实现的功能（支持中文） |
| **4. 查看 Spec** | 流水线自动生成 Spec 到 `.claude/tmp/spec/` | 可打开查看技术规格文档 |
| **5. 确认 Spec / Plan** | Claude 展示摘要并等待确认，可要求修改 | 逐级 Gate 确认，确保方向正确 |
| **6. 自动开发** | 确认后自动创建分支、编写代码、执行 lint | 无需人工介入 |
| **7. 用户验收** | 完成后提示 `pnpm dev` 启动验证 | 确认功能符合预期后可选择合并 |

### 产物目录

每次流水线运行产生的文档存放在 `.claude/tmp/` 目录下：
...
│   ├── <任务名>.md
│   └── <任务名>.state.json
└── pipeline.state.json    # 流水线全局状态（中断恢复用）
```
